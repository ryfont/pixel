...
export function long_description () {
  return m('.col.gap-2.long-description', [
    ...
    m('ol', [
      ...
      m('li', 'Two people click “Become a Debater”, and one of them selects an image by entering keywords for image search or an explicit URL.'),
      ...
      m('li', 'The debaters make their claims about the image (“cat” or “dog”, say), then spend 5 minutes talking and drawing rectangles on the image, trying to convince the judge they are telling the truth.  The judge can also ask questions.  The debaters should take turns, restrict themselves to short statements, and not talk too fast (otherwise, the honest player wins too easily).'),
      m('li', 'A debater can use “Reveal Pixel” to reveal the true value of one pixel to the judge.  Each debater can reveal only one pixel total, so choose wisely.'),
